...
{
    /// <summary>
    /// 세이브 슬롯 씬의 컨트롤러. 슬롯 5칸을 관리한다.
    /// 슬롯 선택 시 비어 있으면 새 게임, 저장되어 있으면 이어하기로 분기한다.
    /// </summary>
    public sealed class SaveSlotScreen : MonoBehaviour
    {
        ...
        private void HandleDeleteRequested(int slot)
        {
            if (confirmDialog == null)
            {
                ...
                Debug.LogError("[SaveSlotScreen] ConfirmDialog가 연결되지 않아 삭제를 취소했습니다.");
                return;
            }
...
#if UNITY_EDITOR
        public void EditorBind(List<SaveSlotView> views, Button back, ConfirmDialog dialog)
        {
            slotViews = views;
            backButton = back;
